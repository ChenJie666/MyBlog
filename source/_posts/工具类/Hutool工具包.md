...
```
对象属性复制：
BeanUtil.copyProperties(dragonflyFms,dragonflyFmVO);

HttpUtil
```
HttpResponse response = HttpUtil.createPost(tokenUrl).execute();
Assert.isTrue(response.isOk(), response.body());
JSONObject resBody = new JSONObject(response.body());
```
发送x-www-form-urlencoded格式的内容
```java
HttpResponse response = HttpUtil.createPost(overflowHost).form(param).execute();
```
